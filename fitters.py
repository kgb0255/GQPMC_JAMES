...
STEP = 1000
print(f'Adaptive Evaluation Step = {STEP}')
...
class iFSPS(Fitter): 
	...
	def MCMC_spectrophoto(self, wave_obs, flux_obs, flux_ivar_obs, photo_obs, photo_ivar_obs, zred, f_fiber_prior=None, mask=None, bands='desi',nwalkers=100, burnin=100, niter=1000, writeout=None, silent=True): 
		...
		chain = self._emcee2(self._lnPost_spectrophoto, lnpost_args, lnpost_kwargs, 
	...
	def MCMC_spec(self, wave_obs, flux_obs, flux_ivar_obs, zred, mask=None, 
		...
		# run emcee and get MCMC chains 
		chain = self._emcee2(self._lnPost, lnpost_args, lnpost_kwargs, 
				nwalkers=nwalkers, burnin=burnin, niter=niter, silent=silent)
		# get quanitles of the posterior
		lowlow, low, med, high, highhigh = np.percentile(chain, [2.5, 16, 50, 84, 97.5], axis=0)
	...
	def MCMC_photo(self, photo_obs, photo_ivar_obs, zred, bands='desi', nwalkers=100, burnin=100, niter=1000, writeout=None, silent=True): 
		''' infer the posterior distribution of the free parameters given observed
		...
		# run emcee and get MCMC chains 
		chain = self._emcee2(self._lnPost_photo, lnpost_args, lnpost_kwargs, 
				nwalkers=nwalkers, burnin=burnin, niter=niter, silent=silent)
		# get quanitles of the posterior
		lowlow, low, med, high, highhigh = np.percentile(chain, [2.5, 16, 50, 84, 97.5], axis=0)
	...
	#JAMES
	def GRD(self,chain,num_chain,length):
		print("GRD called")
		M = num_chain
		N = length
	...
	def _emcee2(self, lnpost_fn, lnpost_args, lnpost_kwargs, nwalkers=100, burnin=100, niter=1000, silent=True): 
		...
		min_result = op.minimize(
				...
				args=lnpost_args,
				method='BFGS',
				options={'epos': 0.01 * dprior, 'maxiter': 100})
		tt0 = min_result['x']
		if not silent: print('initial theta = [%s]' % ', '.join([str(_t) for _t in tt0]))
		...
		for idx in range(niter//STEP):
			...
			convergent,PSRF = self.GRD(result[:,0], nwalkers, STEP * (idx+1))
			pos=pos1
